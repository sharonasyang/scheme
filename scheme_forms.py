...
def do_and_form(expressions, env):
    ...
    while temp_expr is not nil:
        temp_eval = scheme_eval(temp_expr.first, env)
        if is_scheme_false(temp_eval):
            return False
        if temp_expr.rest is nil:
            return temp_eval
...
def make_let_frame(bindings, env):
    ...
    names = values = nil
    # BEGIN PROBLEM 14
    #validate_form(bindings, 2, 2)
    #validate_formals(bindings.rest)
